module "monitor_action_group_process_manager" {
  ...
  email_receiver_name        = "Alerts-${lower(var.domain_name_short)}-${lower(var.environment_short)}-${lower(var.environment_instance)}"
  email_receiver_address     = var.alert_email_address
  custom_dimension_subsystem = ["processmanager"]
  ...
  query_alerts_list = [
    {
      ...
      query       = <<-QUERY
                      exceptions
                        ...
                        | where customDimensions["Subsystem"] == "processmanager"
