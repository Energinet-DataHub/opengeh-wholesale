...
resource "azurerm_monitor_scheduled_query_rules_alert" "edi_internal_commands_alert" {
  name                      = "alert-internal-commands-edi-${lower(var.domain_name_short)}-${lower(var.environment_short)}-${lower(var.environment_instance)}"
  location                  = azurerm_resource_group.this.location
  resource_group_name       = var.shared_resources_resource_group_name
