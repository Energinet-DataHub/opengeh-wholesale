module "sbtsub_edi_enqueue_brs_028" {
  ...
  name               = "edi-enqueue-brs-028"
  topic_id           = data.azurerm_key_vault_secret.sbt_edi_id.value
  project_name       = var.domain_name_short
  max_delivery_count = 10

  correlation_filter = {
    label = "Enqueue_brs_028" # Applies filter to the "subject" of the service bus message. IMPORTANT: This is case sensitive.
  }
}
