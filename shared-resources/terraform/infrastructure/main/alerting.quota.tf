...
resource "azurerm_monitor_scheduled_query_rules_alert_v2" "quota" {
  ...
  action {
    action_groups = [module.monitor_action_group_platform.id]
  }
...
resource "azurerm_role_assignment" "quota_alert_reader_subscription" {
  ...
  principal_id         = azurerm_monitor_scheduled_query_rules_alert_v2.quota.identity[0].principal_id
}
...
resource "azurerm_role_assignment" "quota_alert_reader_subscription_60" {
  scope                = data.azurerm_subscription.this.id
  role_definition_name = "Reader"
  principal_id         = azurerm_monitor_scheduled_query_rules_alert_v2.quota_60.identity[0].principal_id
}
