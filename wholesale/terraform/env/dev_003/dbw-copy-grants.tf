...
resource "databricks_grant" "wholesale_migrations_wholesale_target" {
  provider          = databricks.dbw
  external_location = databricks_external_location.wholesale_migrations_wholesale_copy.id
  principal         = var.databricks_contributor_dataplane_group.name
  privileges        = ["READ_FILES", "WRITE_FILES"]
...
resource "databricks_grant" "wholesale_internal_target" {
  provider          = databricks.dbw
  external_location = databricks_external_location.wholesale_internal_copy.id
  principal         = var.databricks_contributor_dataplane_group.name
  privileges        = ["READ_FILES", "WRITE_FILES"]
