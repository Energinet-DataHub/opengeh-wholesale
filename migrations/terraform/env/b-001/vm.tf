...
resource "azurerm_public_ip" "agent" {
  ...
  name                = "pip-agent${count.index}-${lower(var.domain_name_short)}-${lower(var.environment_short)}-${lower(var.environment_instance)}"
...
resource "azurerm_network_interface" "agent" {
  count               = local.agents_count
  name                = "nic-agent${count.index}-${lower(var.domain_name_short)}-${lower(var.environment_short)}-${lower(var.environment_instance)}"
  resource_group_name = azurerm_resource_group.this.name
  location            = azurerm_resource_group.this.location
...
resource "azurerm_network_security_group" "agent" {
  name                = "nsg-agent-${lower(var.domain_name_short)}-${lower(var.environment_short)}-${lower(var.environment_instance)}"
  location            = azurerm_resource_group.this.location
  resource_group_name = azurerm_resource_group.this.name
...
resource "azurerm_linux_virtual_machine" "agent" {
  count                           = local.agents_count
  name                            = "vm-agent${count.index}-${lower(var.domain_name_short)}-${lower(var.environment_short)}-${lower(var.environment_instance)}"
  resource_group_name             = azurerm_resource_group.this.name
  location                        = azurerm_resource_group.this.location
  ...
  os_disk {
    name                 = "osdisk-agent${count.index}-${lower(var.domain_name_short)}-${lower(var.environment_short)}-${lower(var.environment_instance)}"
    caching              = "ReadWrite"
