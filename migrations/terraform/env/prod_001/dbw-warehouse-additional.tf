resource "databricks_query" "ts_api_sql_endpoint_keep_alive" {
  ...
  display_name = "ts_api_sql_endpoint_keep_alive"
  query_text   = "SELECT 42 as value"
  parent_path  = "/Shared/Queries"
}

resource "databricks_job" "ts_api_sql_endpoint_keep_alive" {
  provider = databricks.dbw
  ...
  task {
    task_key = "ts_api_sql_endpoint_keep_alive"

    sql_task {
      query {
        query_id = databricks_query.ts_api_sql_endpoint_keep_alive.id
      }
      warehouse_id = databricks_sql_endpoint.ts_api_sql_endpoint.id
    }
  }
