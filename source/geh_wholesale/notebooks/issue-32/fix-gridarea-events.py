...
GridAreaTest = spark.read.parquet(
    "abfss://integration-events@stdatalakesharedresu001.dfs.core.windows.net/gridareaEvents-805-806"
).union(
...
old_events_df = (
    spark.read.parquet("abfss://integration-events@stdatalakesharedresu001.dfs.core.windows.net/gridareaEvents-805-806")
    .union(
...
# return
(
    old_events_df.write.mode("overwrite")
...
new_events_df = spark.read.parquet(
    "abfss://integration-events@stdatalakesharedresu001.dfs.core.windows.net/notebook-test"
)

display(new_events_df)
display(
    new_events_df.withColumn("body", col("body").cast("string")).withColumn("body", from_json(col("body"), grid_schema))
)


# COMMAND ----------

GridAreaEventsDf = spark.read.parquet(
    "abfss://integration-events@stdatalakesharedresu001.dfs.core.windows.net/gridareaEvents-805-806"
)
display(GridAreaEventsDf)
GridAreaEventsDf.write.option("mergeSchema", "true").mode("append").parquet(
    "abfss://integration-events@stdatalakesharedresu001.dfs.core.windows.net/events"
)
...
new_df = (
    ...
    .withColumn(
        "GridAreaId",
        when(col("GridAreaCode") == 805, "89801EC1-AF12-46D9-B044-05A004A0D46C").otherwise(
            "696E5DBA-B028-4A5D-B8B4-E0C01AFD3CFE"
        ),
    )
    .withColumn(
        "body",
        when(
            col("MessageType").startswith("Grid"),
            to_json(
                struct(
                    col("MessageType"),
                    col("GridAreaLinkId"),
        ...
        .when(
            col("MessageType").startswith("MeteringPoint"),
            to_json(
                struct(
                    col("MessageType"),
                    col("GsrnNumber"),
        ...
        .otherwise(to_json(struct(col("MessageType"), col("GridAreaCode"))).cast("binary")),
    )
    .select(
        col("partition"),
        col("offset"),
...
new_df.write.mode("overwrite").partitionBy("year", "month", "day").parquet(
    "abfss://integration-events@stdatalakesharedresu001.dfs.core.windows.net/notebook-test"
)
