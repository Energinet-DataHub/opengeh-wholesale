...
def initialize_spark(args):
    ...
    spark_conf = (
        ...
        .set(
            "fs.azure.account.key.{0}.dfs.core.windows.net".format(
...
p = configargparse.ArgParser(
    description="Green Energy Hub Tempory aggregation triggger",
...
p.add(
    ...
    type=str,
    required=True,
    help="Azure Storage key for storage",
    env_var="GEH_INPUT_STORAGE_KEY",
)
p.add(
...
p.add(
    ...
    type=str,
    required=False,
    default="delta/time_series_test_data/",
...
p.add(
    "--result-url", type=str, required=True, help="The target url to post result json"
),
p.add(
    "--result-id",
    type=str,
    required=True,
    help="Postback id that will be added to header",
),
p.add("--persist-source-dataframe", type=bool, required=False, default=False)
p.add(
    "--persist-source-dataframe-location",
    type=str,
    required=False,
    default="delta/basis-data/",
)
p.add(
    "--snapshot-url", type=str, required=True, help="The target url to post result json"
)
p.add(
    "--cosmos-account-endpoint", type=str, required=True, help="Cosmos account endpoint"
)
...
p.add(
    "--cosmos-container-metering-points",
    type=str,
    required=True,
    help="Cosmos container for metering points input data",
)
p.add(
    "--cosmos-container-market-roles",
    type=str,
    required=True,
    help="Cosmos container for market roles input data",
)
p.add(
    "--cosmos-container-grid-loss-sys-corr",
    type=str,
    required=True,
    help="Cosmos container for grid loss and system correction",
)
p.add(
    "--cosmos-container-es-brp-relations",
    type=str,
    required=True,
    help="Cosmos container for relations between energy supplier and balance responsible",
)
p.add(
    "--resolution",
    type=str,
    required=True,
    help="Time window resolution eg. 1 hour, 15 minutes etc.",
)
...
def load_metering_points(args, spark):
    return load_aggregation_data(
        args.cosmos_container_metering_points, metering_point_schema, args, spark
    )
...
def load_time_series(args, areas, spark):
    ...
    TIME_SERIES_STORAGE_PATH = "abfss://{0}@{1}.dfs.core.windows.net/{2}".format(
        args.data_storage_container_name,
        args.data_storage_account_name,
    ...
    valid_time_period_df = filter_time_period(
        timeseries_df, beginning_date_time, end_date_time
    )
    # Filter out time series data that do not belong to the specified grid areas
    if areas:
        valid_time_period_df = valid_time_period_df.filter(col("grid_area").isin(areas))
...
def get_time_series_dataframe(args, areas, spark):
    ...
    time_series_with_metering_point = (
        time_series_df.join(metering_point_df, metering_point_join_conditions)
