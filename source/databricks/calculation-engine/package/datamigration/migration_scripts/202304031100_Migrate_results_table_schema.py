...
def apply(args: MigrationScriptArgs) -> None:
    "Migration converts the `quantity` column from string to decimal(18,3)"

    delta_table_path = _get_delta_table_path(args.storage_account_name)
    results_df = _get_results_df(args.spark, delta_table_path)

    results_df = results_df.withColumn(
        "quantity", col("quantity").cast("decimal(18,3)")
    )
...
def _get_delta_table_path(storage_account_name: str) -> str:
    return f"abfss://{CONTAINER}@{storage_account_name}.dfs.core.windows.net/{DIRECTORY_NAME}/{RESULT_TABLE_NAME}"
