-- TODO BJM: This is an experiment regarding the performance of the settlement report.
--           If accepted we should create a job for the optimization and consider whether to
--           replace this materialized view by extending the time_series_points table with the joined
--           columns and add the data as part of the write in the calculation engine.

CREATE MATERIALIZED VIEW {HIVE_SETTLEMENT_REPORT_DATABASE_NAME}.metering_point_time_series_v2 AS
...
CLUSTER BY (calculation_id, calculation_type, grid_area_code, energy_supplier_id, observation_time)
GO

OPTIMIZE {HIVE_SETTLEMENT_REPORT_DATABASE_NAME}.metering_point_time_series_v2
