...
def round_quantity(df: DataFrame) -> DataFrame:
    ...
    df = df.sort_values(Colname.observation_time)
    df["index"] = df[Colname.observation_time].dt.minute / 15 + 1
    df["quantity_row_1"] = df[Colname.quantity].astype(float)
    print(df.quantity_row_1)
    df["quantity_row_2"] = (
        df[Colname.quantity].astype(float)
        - df.quantity_row_1.astype(float).round(3)
        + df.quantity_row_1.astype(float)
    )
    print(df.quantity_row_2)
    df["quantity_row_3"] = (
        df[Colname.quantity].astype(float)
        - df.quantity_row_2.astype(float).round(3)
        + df.quantity_row_2.astype(float)
    )
    print(df.quantity_row_3)
    df["quantity_row_4"] = (
        df[Colname.quantity].astype(float)
        - df.quantity_row_3.astype(float).round(3)
        + df.quantity_row_3.astype(float)
    )
    print(df.quantity_row_4)
    df["round_ready_quantity"] = np.where(
        df["index"] == 1,
        df["quantity_row_1"],
        np.where(
            df["index"] == 2,
            ...
            np.where(
                df["index"] == 3,
                df["quantity_row_3"],
                np.where(df["index"] == 4, df["quantity_row_4"], df["index"]),
            ),
        ),
    )
    df[Colname.quantity] = df.round_ready_quantity.astype(float).round(3)
