...
FROM {BASIS_DATA_DATABASE_NAME}.metering_point_periods AS m
         JOIN (SELECT * FROM {BASIS_DATA_DATABASE_NAME}.time_series_points order by t.observation_time) AS t ON m.metering_point_id = t.metering_point_id AND m.calculation_id = t.calculation_id
