...
FROM basis_data.charge_masterdata_periods as cm
INNER JOIN basis_data.charge_price_points as cp ON cm.calculation_id = cp.calculation_id AND cm.charge_key = cp.charge_key
INNER JOIN (
  SELECT distinct mp.calculation_id, charge_key, energy_supplier_id, grid_area_code FROM basis_data.charge_link_periods AS cl
  INNER JOIN basis_data.metering_point_periods AS mp ON mp.calculation_id = cl.calculation_id AND mp.metering_point_id = cl.metering_point_id
) AS es_ga ON cm.calculation_id = es_ga.calculation_id AND cm.charge_key = es_ga.charge_key
INNER JOIN basis_data.calculations AS c ON cm.calculation_id = c.calculation_id
WHERE cm.calculation_id = "46ec14d2-4693-4ef9-b66f-edda23824b68"
