...
from package.settlement_report_job.settlement_report import start, start_with_deps
...
class TestWhenInvokedWithValidArguments:
    def test_does_not_raise(
        ...
        command_line_args.report_id = any_settlement_report_args.report_id

        # Act
        start_with_deps(
            parse_command_line_args=lambda: command_line_args,
            parse_job_args=lambda args: any_settlement_report_args,
        )
    ...
    def test_add_info_log_record_to_azure_monitor_with_expected_settings(
        ...
        - custom field "report_id" = <the report id>
        ...
        # Arrange
        self.prepare_command_line_arguments(any_settlement_report_args)
...
| where Properties.report_id == "{any_settlement_report_args.report_id}"
| where Properties.CategoryName == "Energinet.DataHub.package.settlement_report_args"
| count
        """
...
| where Properties.Subsystem == "wholesale-aggregations"
| where Properties.report_id == "{any_settlement_report_args.report_id}"
| where Properties.CategoryName == "Energinet.DataHub.package.settlement_report_job"
| count
        """
    ...
    @staticmethod
    def prepare_command_line_arguments(any_settlement_report_args):
        any_settlement_report_args.report_id = str(uuid.uuid4())
        sys.argv = []
        sys.argv.append("--dummy=")
        sys.argv.append(f"--report-id={str(any_settlement_report_args.report_id)}")
        sys.argv.append("--period-start=2023-01-31T23:00:00Z")
        sys.argv.append("--period-end=2023-01-31T23:00:00Z")
