...
from business_logic_tests.features.basic.states.state import get_expected_result
...
def test_execute__returns_expected(
    ...
    # Arrange
    scenario_fixture.setup(file_path=__file__, expected_result=get_expected_result)
    ...
    # Assert
    results = (
        results.energy_results.consumption_per_ga.df.drop("metering_point_type")
    ...
    expected_results = (
        scenario_fixture.expected_results.energy_results.consumption_per_ga.df.drop(
            "metering_point_type"
        )
        .drop("quantity_qualities")
        .drop("price")
