...
class SettlementReportViewReader:
    ...
    @staticmethod
    def read_energy_result_points_per_ga_v1(spark: SparkSession) -> DataFrame:
        return spark.read.format("delta").table(
            f"{SETTLEMENT_REPORT_DATABASE_NAME}.{ENERGY_RESULT_POINTS_PER_GA_SETTLEMENT_REPORT_VIEW_NAME_V1}"
        )

    @staticmethod
    def read_energy_result_points_per_es_ga_v1(spark: SparkSession) -> DataFrame:
        return spark.read.format("delta").table(
            f"{SETTLEMENT_REPORT_DATABASE_NAME}.{ENERGY_RESULT_POINTS_PER_ES_GA_SETTLEMENT_REPORT_VIEW_NAME_V1}"
        )
    ...
    @staticmethod
    def read_monthly_amounts_v1(spark: SparkSession) -> DataFrame:
        return spark.read.format("delta").table(
            f"{SETTLEMENT_REPORT_DATABASE_NAME}.{MONTHLY_AMOUNTS_SETTLEMENT_REPORT_VIEW_NAME_V1}"
        )
