...
class SettlementReportViewReader:
    ...
    @staticmethod
    def read_energy_result_points_per_ga_es_v1(spark: SparkSession) -> DataFrame:
        return spark.read.format("delta").table(
            f"{SETTLEMENT_REPORT_DATABASE_NAME}.{ENERGY_RESULT_POINTS_PER_GA_ES_SETTLEMENT_REPORT_VIEW_NAME_V1}"
        )
