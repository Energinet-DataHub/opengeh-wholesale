...
def _get_base_energy_row_data_by_spec(
    current_time: datetime, data_spec: EnergyTestDataSpec
):
    return {
...
def create_energy_per_es_v1(
    ...
    while current_time < data_spec.to_date:
        rows.append(
            _get_base_energy_row_data_by_spec(current_time, data_spec=data_spec)
        )
