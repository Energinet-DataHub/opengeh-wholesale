...
from package.shared.services import InputOutputProcessor
from package.shared.data_loader import (
...
def create_snapshot(spark: SparkSession, areas, args: dict):
    ...
    metering_points = load_metering_points(
        args.beginning_date_time, args.end_date_time, args, spark, areas
    )
    snapshot_data[BasisDataKeyName.metering_points] = metering_points

    # Fetch time series dataframe
    snapshot_data[BasisDataKeyName.time_series] = load_time_series_points(
        args, spark, metering_points
    )
    ...
    snapshot_data[BasisDataKeyName.es_brp_relations] = load_es_brp_relations(
        args, spark, areas
    )
    ...
    snapshot_data[BasisDataKeyName.grid_loss_sys_corr] = load_grid_loss_sys_corr(
        args, spark, areas
    )
