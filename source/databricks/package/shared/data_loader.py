...
from package.shared.filters import (
    filter_on_date,
    filter_on_period,
...
from package.shared.services import StorageAccountService
from package.shared.period import Period, parse_period
...
def __load_delta_data(
    spark: SparkSession,
    storage_base_path: str,
    delta_table_path: str,
    where_condition: str = None,
) -> DataFrame:
    path = StorageAccountService.get_storage_account_full_path(
        storage_base_path, delta_table_path
    )
...
def __load_from_sql_table(
    spark: SparkSession, args: Namespace, table_name: str
) -> DataFrame:
    return (
        spark.read.format("com.microsoft.sqlserver.jdbc.spark")
        .option(
            "url",
            f"jdbc:sqlserver://{args.shared_database_url};databaseName={args.shared_database_aggregations};",
        )
...
def load_metering_points(
    ...
    df = (
        __load_from_sql_table(spark, args, "MeteringPoint")
...
def load_grid_loss_sys_corr(
    args: Namespace, spark: SparkSession, grid_areas: List[str]
) -> DataFrame:
    df = (
        __load_from_sql_table(spark, args, "GridLossSysCorr")
        .withColumnRenamed("MeteringPointId", Colname.metering_point_id)
        .withColumnRenamed("GridArea", Colname.grid_area)
    ...
    df = filter_on_period(
        df, parse_period(args.beginning_date_time, args.end_date_time)
    )
    df = filter_on_grid_areas(df, Colname.grid_area, grid_areas)
    return df
...
def load_market_roles(args: Namespace, spark: SparkSession) -> DataFrame:
    ...
    hardcoded_energy_suppliers = [
        ("42", "some-mp-id", "2010-01-01T00:00:00Z", "2021-01-01T00:00:00Z")
    ]
...
def load_charges(args: Namespace, spark: SparkSession) -> DataFrame:
    df = (
        __load_from_sql_table(spark, args, "Charge")
...
def load_charge_links(args: Namespace, spark: SparkSession) -> DataFrame:
    df = (
        __load_from_sql_table(spark, args, "ChargeLink")
        .withColumnRenamed("ChargeKey", Colname.charge_key)
        .withColumnRenamed("MeteringPointId", Colname.metering_point_id)
        .withColumnRenamed("FromDate", Colname.from_date)
        .withColumnRenamed("ToDate", Colname.to_date)
...
def load_charge_prices(args: Namespace, spark: SparkSession) -> DataFrame:
    df = (
        __load_from_sql_table(spark, args, "ChargePrice")
        .withColumnRenamed("ChargeKey", Colname.charge_key)
...
def load_es_brp_relations(
    ...
    hardcoded_energy_suppliers = [
        ("brp-id-43", "123", "2010-01-01T00:00:00Z", "2021-01-01T00:00:00Z")
    ]
    df = spark.sparkContext.parallelize(hardcoded_energy_suppliers).toDF(columns)
    df = df.withColumn(Colname.from_date, F.to_timestamp(Colname.from_date))
    df = df.withColumn(Colname.to_date, F.to_timestamp(Colname.to_date))
...
def load_time_series_points(
    args: Namespace, spark: SparkSession, metering_point_df: DataFrame
) -> DataFrame:
    df = __load_delta_data(
        ...
        time_series_points_where_date_condition(
            parse_period(args.beginning_date_time, args.end_date_time)
        ),
    )
    ...
    df = filter_time_series_by_metering_points(
        df, metering_point_df.select(col(Colname.metering_point_id))
    )
...
def select_latest_point_data(df: DataFrame) -> DataFrame:
    df = df.withColumn(
        "row_number",
        F.row_number().over(
            Window.partitionBy(Colname.metering_point_id, Colname.time).orderBy(
                F.col(Colname.registration_date_time).desc()
            )
        ),
    )
...
def filter_time_series_by_metering_points(
    df: DataFrame, metering_point_df: DataFrame
) -> DataFrame:
    # Solely include time series for which we have metering point master data
    return df.join(
        metering_point_df,
        df.metering_point_id == metering_point_df.metering_point_id,
        "leftsemi",
    )
