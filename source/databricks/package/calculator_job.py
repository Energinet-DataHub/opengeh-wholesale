...
def _check_all_grid_areas_have_metering_points(
    batch_grid_areas_df: DataFrame, metering_points_periods_df: DataFrame
) -> None:
    distinct_grid_areas_rows_df = (
        metering_points_periods_df.withColumnRenamed("GridArea", "GridAreaCode")
        .select("GridAreaCode")
        .distinct()
    )
