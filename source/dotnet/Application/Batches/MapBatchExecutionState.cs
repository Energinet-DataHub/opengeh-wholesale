...
{
    public async Task<IEnumerable<Batch>> MapExecutionStatesAsync(IBatchRepository batchRepository, ICalculatorJobRunner calculatorJobRunner)
    {
        ...
        foreach (var batch in pendingAndExecutingBatches)
        {
            ...
            switch (state)
            {
                case JobState.Running:
                    if (batch.ExecutionState != BatchExecutionState.Executing)
