    ...
    DateTimeOffset endDate)
{
    ...
    public IEnumerable<string> GridAreaCodes { get; } = gridAreaCodes;

    public DateTimeOffset StartDate { get; } = startDate;

    public DateTimeOffset EndDate { get; } = endDate;

    public CalculationRequest(CalculationRequestDto calculationRequestDto, Guid requestedByUserId)
        : this(requestedByUserId, calculationRequestDto.CalculationType, calculationRequestDto.GridAreaCodes, calculationRequestDto.StartDate, calculationRequestDto.EndDate)
    {
    }
}
