...
{
    public async Task WriteRecordsAsync<T>(Stream destination, IEnumerable<T> records)
    {
        var writer = new StreamWriter(destination);

        await using (writer.ConfigureAwait(false))
        {
            var csvHelper = new CsvHelper.CsvWriter(writer, CultureInfo.InvariantCulture);

            await using (csvHelper.ConfigureAwait(false))
            {
                await csvHelper.WriteRecordsAsync(records).ConfigureAwait(false);
            }
        }
    }
}
