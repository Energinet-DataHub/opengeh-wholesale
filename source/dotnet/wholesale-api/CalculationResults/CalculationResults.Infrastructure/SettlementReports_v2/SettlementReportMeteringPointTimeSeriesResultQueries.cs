...
public sealed class SettlementReportMeteringPointTimeSeriesResultQueries : ISettlementReportMeteringPointTimeSeriesResultQueries
{
    ...
    public async Task<int> CountAsync(SettlementReportMeteringPointTimeSeriesResultQueryFilter filter)
    {
        ...
        {
            return SqlResultValueConverters.ToInt(new DatabricksSqlRow(nextRow)["count"])!.Value;
        }

        throw new InvalidOperationException($"Could not count result for {nameof(SettlementReportMeteringPointTimeSeriesResultQueries)}.");
    }
