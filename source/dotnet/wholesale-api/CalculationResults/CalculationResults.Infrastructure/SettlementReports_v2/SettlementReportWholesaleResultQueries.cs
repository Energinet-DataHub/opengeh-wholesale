...
public class SettlementReportWholesaleResultQueries
{
    ...
    public async IAsyncEnumerable<SettlementReportWholesaleResultRow> GetAsync(Guid calculationId)
    {
        var calculation = await _calculationsClient.GetAsync(calculationId).ConfigureAwait(false);
        var statement = new SettlementReportWholesaleResultQueryStatement(calculationId);
        ...
        {
            yield return SettlementReportResultRowFactory.Create(new DatabricksSqlRow(nextRow), calculation.Version);
        }
    }
}
