...
public class WholesaleResultColumnNameTests
{
    [Fact]
    public async Task WholesaleResultColumnNames_MatchesContract()
    {
        // Arrange
        await using var stream = EmbeddedResources.GetStream<Root>("DeltaTableContracts.wholesale-result-table-column-names.json");
        ...
        var actualColumnTypes = actualColumnNames.Select(WholesaleResultColumnNames.GetType).ToList();
