...
public class SqlStatementFactoryTests
{
    ...
    public void CreateForSettlementReport_ReturnsExpectedSqlStatement()
    {
        ...
        const string expectedSql = "SELECT grid_area, batch_process_type, time, time_series_type, quantity FROM wholesale_output.result WHERE grid_area IN (123,234,345) AND time_series_type IN ('production','flex_consumption','non_profiled_consumption','net_exchange_per_ga') AND batch_process_type = 'BalanceFixing' AND time BETWEEN '2022-10-12T01:00:00Z' AND '2022-10-12T03:00:00Z' AND aggregation_level = 'total_ga' order by time";
