...
public class JobsApiClient : ApiClient, IJobsApi
{
    ...
    public async Task<RunIdentifier> RunNow(long jobId, RunParameters runParams,
        CancellationToken cancellationToken = default)
    {
        var settings = new RunNowSettings { JobId = jobId };
    ...
    {
        var result =
            await HttpPost<RunOnceSettings, RunIdentifier>(this.HttpClient, "jobs/runs/submit", settings,
                    cancellationToken)
                .ConfigureAwait(false);
        return result.RunId;
    }
    ...
    public async Task<(string, string, Run)> RunsGetOutput(long runId, CancellationToken cancellationToken = default)
    {
        ...
        string notebookOutput = PropertyExists(response, "notebook_output") &&
                                PropertyExists(response.notebook_output, "result")
