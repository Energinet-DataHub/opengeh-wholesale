...
public class SecretsApiClient : ApiClient, ISecretsApi
{
    ...
    */
    ...
    private static readonly JsonSerializer SecretScopeConverter = new() { Converters = { new SecretScopeConverter() } };

    public async Task<IEnumerable<SecretScope>> ListScopes(CancellationToken cancellationToken = default)
    {
        var scopeList = await HttpGet<dynamic>(this.HttpClient, "secrets/scopes/list", cancellationToken)
