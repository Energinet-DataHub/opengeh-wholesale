...
public sealed class DbfsApiClient : ApiClient, IDbfsApi
{
    ...
    public async Task<long> Create(string path, bool overwrite, CancellationToken cancellationToken = default)
    {
        ...
        var response = await HttpPost<dynamic, FileHandle>(this.HttpClient, "dbfs/create", request, cancellationToken)
    ...
    public async Task Upload(string path, bool overwrite, Stream stream, CancellationToken cancellationToken = default)
    {
        ...
        var handle = await Create(path, overwrite, cancellationToken).ConfigureAwait(false);
        ...
        try
        {
            ...
            while ((bytesRead = await stream.ReadAsync(buffer, 0, mb, cancellationToken)) > 0)
            {
                ...
                await AddBlock(handle, contents, cancellationToken).ConfigureAwait(false);
            }
