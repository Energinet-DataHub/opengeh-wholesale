...
public class UpdateBatchExecutionStateEndpointTests
{
    [Fact]
    public async Task ServiceCollection_CanResolveZipBasisEndpointTests()
    {
        // Arrange
        using var host = await ProcessManagerIntegrationTestHost
            .CreateAsync(collection => collection.AddScoped<UpdateBatchExecutionStateEndpoint>());

        await using var scope = host.BeginScope();

        // Act & Assert that the container can resolve the endpoints dependencies
        scope.ServiceProvider.GetRequiredService<UpdateBatchExecutionStateEndpoint>();
    }
}
