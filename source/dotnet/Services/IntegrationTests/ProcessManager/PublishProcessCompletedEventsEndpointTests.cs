...
public class PublishProcessCompletedEventsEndpointTests
{
    [Fact]
    public async Task ServiceCollection_CanResolvePublishProcessCompletedEventsEndpoint()
    {
        // Arrange
        using var host = await ProcessManagerIntegrationTestHost
            .CreateAsync(collection => collection.AddScoped<PublishProcessCompletedEventsEndpoint>());

        await using var scope = host.BeginScope();

        // Act & Assert that the container can resolve the endpoints dependencies
        scope.ServiceProvider.GetRequiredService<PublishProcessCompletedEventsEndpoint>();
    }
}
