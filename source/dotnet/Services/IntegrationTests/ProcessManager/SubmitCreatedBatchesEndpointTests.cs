...
{
    [Fact]
    public async Task ServiceCollection_CanResolveSubmitCreatedBatchesEndpointTests()
    {
        // Arrange
        using var host = await ProcessManagerIntegrationTestHost
            .CreateAsync(collection => collection.AddScoped<SubmitCreatedBatchesEndpointTests>());

        await using var scope = host.BeginScope();

        // Act & Assert that the container can resolve the endpoints dependencies
        scope.ServiceProvider.GetRequiredService<SubmitCreatedBatchesEndpointTests>();
    }
}
