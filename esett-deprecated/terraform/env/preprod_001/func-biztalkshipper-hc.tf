resource "azurerm_function_app_hybrid_connection" "biztalkshipper_biztalk" {
  ...
  provisioner "local-exec" {
    command = "az functionapp hybrid-connection add --hybrid-connection ${data.azurerm_key_vault_secret.hc_biztalk_name.value} --namespace ${data.azurerm_key_vault_secret.relay_name.value} -n ${module.func_biztalkreceiver.name} -g ${azurerm_resource_group.this.name} --subscription ${var.subscription_id}"
  }
}
