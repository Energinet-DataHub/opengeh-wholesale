resource "databricks_job" "missing_measurements_log" {
  provider            = databricks.dbw
  name                = "MissingMeasurementsLog"
  max_concurrent_runs = 1

  job_cluster {
    job_cluster_key = "missing_measurements_log_cluster"
  ...
  task {
    task_key        = "missing_measurements_log_task"
    max_retries     = 1
    job_cluster_key = "missing_measurements_log_cluster"
    ...
    python_wheel_task {
      ...
      entry_point = "execute_missing_measurements_log"
    }
  }
...
resource "databricks_permissions" "missing_measurements_log" {
  provider = databricks.dbw
  job_id   = databricks_job.missing_measurements_log.id
